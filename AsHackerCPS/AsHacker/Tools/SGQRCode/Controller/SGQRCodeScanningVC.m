...
//        [self.navigationController pushViewController:jumpVC animated:YES];
        ...
        [self checkCode:[obj stringValue]];
...
//29. 验证入场码
//入口：/m/api/checkEntercode
//请求头：Content-Type: application/x-www-form-urlencoded
//authorization，同4
//请求方法：post
//上传参数：PROJECT_ID=当前项目ID & ENTER_CODE=入场码
//返回：
//{"statusCode":1} 当前用户未登录
//401 Unauthorized 用户不正确
//成功：{"statusCode":0}
//验证失败：{"statusCode":2}
-(void)checkCode:(NSString *)code{
    
    [MOMNetWorking asynRequestByMethod:@"checkEntercode" params:@{@"PROJECT_ID":[_dataDic objectForKey:@"PROJECT_ID"],@"ENTER_CODE":code} publicParams:MOMNetPublicParamUserId callback:^(id result, NSError *error) {
...
//            NSArray *arr1 = [dic objectForKey:@"preResults"];
            ...
            [MOMProgressHUD showSuccessWithStatus:[NSString stringWithFormat:@"扫码通过 %@",code]];
        }else{
            [MOMProgressHUD showSuccessWithStatus:@"扫码未通过"];
        }
    }];
