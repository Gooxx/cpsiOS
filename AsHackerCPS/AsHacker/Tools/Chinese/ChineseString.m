...
+(NSMutableArray*)LetterSortArray:(NSArray*)stringArr
{
    ...
    for (NSObject* object in tempArray) {
        NSString *pinyin = [((ChineseString*)object).pinYin substringToIndex:1];
        CarBrandModel *book=((ChineseString*)object).book;
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        CarBrandModel *book=[stringArr objectAtIndex:i];
        chineseString.book=book;
        chineseString.string=[NSString stringWithString:book.brand_cnName];
...
//                                                pinyinFirstLetter([chineseString.string characterAtIndex:j])]uppercaseString];
                    NSString *singlePinyinLetter=[[book.brand_enName substringToIndex:1]uppercaseString];
