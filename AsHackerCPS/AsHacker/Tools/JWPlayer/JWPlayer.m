...
//        [self.playProgress setThumbImage:[UIImage imageNamed:@"MoviePlayer_Slider"] forState:UIControlStateNormal];
        ...
        if(frame.size.width==500){
            CGRect pv =  self.playerView.layer.bounds;
            
            
            pv.size.height =  pv.size.height*1.5;
            pv.size.width =  pv.size.width*1.5;
            pv.origin.x= pv.origin.x-100;
            pv.origin.y= pv.origin.y-100;
            _playerLayer.frame = pv;//CGRectMake(0, -100,pv.size.width, 470);
        }else{
        ...
        */
//        _playerLayer.frame = CGRectMake(0, 0, 1000, 274);
...
- (void)updateVideoSlider:(float)currentTime{
    ...
    float loseTime = self.playProgress.maximumValue - currentTime;
    self.currentTimeLabel.text = [self convertTime:loseTime];
